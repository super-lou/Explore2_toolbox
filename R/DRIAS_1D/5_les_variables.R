...
debit.name = "debit"
debit.dimension = "station, time"
debit.precision = "float"
debit.value =
    matrix(data=round(runif(length(time.value)*length(station.value)),
                      2),
           ncol=length(time.value))
debit.01.standard_name = "debit"
debit.02.long_name = "debit modcou"
debit.03.units = "m3.s-1"
debit.04.FillValue = "NA"
debit.05.missing_value = "NA"
debit.06.cell_methods = "time:sum"
