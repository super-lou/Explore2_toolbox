...
SWI.name = "SWI"
SWI.dimension = "time, y, x"
SWI.precision = "float"
SWI.value = array(data=round(runif(length(x.value) *
                                      length(y.value) *
                                      length(time.value), 0, 10)),
                     dim=c(length(x.value),
                           length(y.value),
                           length(time.value)))
SWI.01.standard_name = "SWI"
SWI.02.long_name = "Soil Water Index"
SWI.03.grid_mapping = "LambertParisII"
SWI.04.units = " "
SWI.05.coordinates = "lat lon"
SWI.06.FillValue = "NA"
SWI.07.missing_value = "NA"
SWI.08.cell_methods = "time:mean"
SWI.09.comment = "Potential evapotranspiration calculated using the Hargreaves method with unique coefficient 0.175 from DRIAS-2020 corrected data set (the variable rsdsAdjust is not used). Source : ........."
